...
if modkind == 'reg':
    ...
    models = {'clf-LR': LogisticRegressionCV(random_state=123, penalty='elasticnet', n_jobs=2,max_iter=1000,
             ...
             'clf-SVC': GridSearchCV(SVC(kernel='linear',random_state=123),SVR_PG,n_jobs=3, scoring='balanced_accuracy'),
             'clf-RFC': GridSearchCV(RandomForestClassifier(random_state=123,
                                                      oob_score=True),
                                 RFR_PG,n_jobs=3,scoring='balanced_accuracy'),
    ...
    options = [['all','ROSMAP'],
              ...
              ['I-IIpos','I-IVpos','InfTemppos'],
               ['classic','multises'],
               ['None','SelPerc','SelFDR','PCA'],
               ['clf-LR','clf-SVC','clf-RFC','clf-XGBC']]
